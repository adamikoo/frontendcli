package com.antigravity.clifrontend.api
...
class BridgeClient(var baseUrl: String = "http://127.0.0.1:8765") {
        ...
        executor.execute {
            try {
                ...
                if (conn.responseCode == 200) {
                        ...
                        if (l.startsWith("data: ")) {
                            ...
                            try {
                                ...
                                if (json.optString("event") == "exit") {
                                    ...
                                    runOnMain({ onComplete(exitCode) }, Unit)
                                } else {
                                    runOnMain({ onEvent(json) }, Unit)
                                }
                            } catch (e: Exception) {
                                // Skip non-json lines
                            }
                    ...
                    runOnMain({ onError("Server returned HTTP ${conn.responseCode}") }, Unit)
                }
            } catch (e: Exception) {
                runOnMain({ onError(e.message ?: "Connection error") }, Unit)
            }
        }
    }
}
