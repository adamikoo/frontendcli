...
# Check if running in native Termux vs Ubuntu PRoot
if [ -d "/data/data/com.termux" ] && [ ! -f "/etc/os-release" ]; then
    echo "=================================================="
    echo "CLIFrontend Bridge: Native Termux environment"
    echo "Switching to Ubuntu PRoot environment for glibc..."
    echo "=================================================="

    # 1. Ensure proot-distro is installed
    if ! command -v proot-distro >/dev/null 2>&1; then
        echo "[INFO] Installing proot-distro in Termux..."
        pkg update -y && pkg install -y proot-distro || {
            echo "[ERROR] Failed to install proot-distro in Termux."
            exit 1
        }
    fi

    # 2. Ensure Ubuntu rootfs is installed
    if ! proot-distro list 2>/dev/null | grep -qi "ubuntu.*installed"; then
        echo "[INFO] Installing Ubuntu container in proot-distro..."
    ...
    # 3. Ensure python3 is installed in Ubuntu
    proot-distro login ubuntu -- bash -c "command -v python3 >/dev/null 2>&1 || (apt-get update && apt-get install -y python3 curl)"

    # 4. Acquire termux wake lock
    if command -v termux-wake-lock >/dev/null 2>&1; then
        termux-wake-lock 2>/dev/null || true
    fi

    # 5. Exec inside Ubuntu PRoot container with shared storage
    echo "[INFO] Launching CLIFrontend Bridge in Ubuntu PRoot..."
    exec proot-distro login ubuntu --bind /sdcard:/sdcard --bind "$HOME:/root/termux_home" -- "$0" "$@"
fi
